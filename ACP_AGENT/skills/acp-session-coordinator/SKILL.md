...
Use `--member-token-ref env:NAME` when generating an existing listener config so the ACP member token stays outside the file; the variable is resolved only when the wait starts.
