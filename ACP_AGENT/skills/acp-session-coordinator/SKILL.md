...
For deterministic roadmap continuation, the product may additionally set both `coordinator_plan_definition_path` and `coordinator_plan_state_path` (or pass `--plan-definition` plus `--plan-state`). ACP never invents the roadmap: the definition declares task owner, dependencies, risk, and approval gate. A terminal result advances only when its sender matches the dispatched owner; high/sensitive risk requires explicit approval. The next TASK uses a deterministic id, is marked sent only after Hub acceptance, and the source result is ACKed last. Unstructured INFO keeps the compatible wrapper path. Empty waits do not load the plan or call a host/model.
