...
import { buildInvitePrompt } from './invitePrompt.ts'
...
test('buildInvitePrompt creates a self-contained safe invitation for an external LLM agent', () => {
  ...
  assert.match(prompt, /ACP client 0\.3\.15 or newer/)
  ...
  assert.match(prompt, /join-session \\/)
  ...
  assert.match(prompt, /runner start \\/)
