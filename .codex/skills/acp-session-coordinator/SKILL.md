...
Generate/migrate profiles idempotently with `host-bridge configure --role worker|coordinator --adapter-id <id> --host-thread-id <id> [--host-executable <abs>] [--coordinator <name>] [--check]` (no host call, no new session): a `worker` sets `host_bridge_reply_to = codex-pilot-reply-collector` and authorizes its coordinator; a `coordinator` authorizes the collector and never routes replies back to it. Or set keys manually: `host_bridge_adapter_id`, loopback `host_bridge_endpoint`, `host_bridge_allowed_senders`, and `host_bridge_session_id` for OpenCode/Kilo/Claude or `host_bridge_thread_id` for Codex.
...
Optional `host_bridge_credential_ref: "env:NAME"` resolves JSON `username`/`password` for OpenCode/Kilo or `bearer_token` for Codex/Claude; host timeout is capped at 240 and shrinks to preserve REPLY/ACK before lease expiry. Optional `host_bridge_reply_to` (or `--reply-to`) routes plain-TASK replies to a reply-collector member (e.g. `codex-pilot-reply-collector`); a wrapped REPLY/INFO still answers the original worker so the coordinator never loops back into the collector.
...
Generate the supervisor config for all configured components with `python ACP_AGENT/acp.py host-supervisor generate --output ACP_AGENT/agents/supervisor.json --coordinator <name> --worker <name> --reply-collector <name>` (reuses the non-model supervisor; `autostart: not_installed`). Operational supervisor commands: `python ACP_AGENT/acp.py host-supervisor once --config ACP_AGENT/agents/supervisor.json`, then `start`/`stop` with the same explicit config.
When the destination is TASK-only, start the separate collector with `python ACP_AGENT/acp.py reply-collector start --config ACP_AGENT/agents/coordinator-reply-collector.json --forward-action TASK`; the wrapper preserves original REPLY/INFO metadata and ACK remains post-forward.
For deterministic roadmap continuation, the product may additionally set both `coordinator_plan_definition_path` and `coordinator_plan_state_path` (or pass `--plan-definition` plus `--plan-state`). ACP never invents the roadmap: the definition declares task owner, dependencies, risk, approval gate, and optional `max_attempts` (default 1, maximum 5). A terminal result advances only when its sender matches the dispatched owner; high/sensitive risk requires explicit approval. A failed/interrupted task only emits another attempt when the product declared a retry budget; attempts use distinct deterministic IDs while crash replay inside one attempt reuses its ID. The next TASK is marked sent only after Hub acceptance, and the source result is ACKed last. Unstructured INFO keeps the compatible wrapper path. Empty waits do not load the plan or call a host/model. A structured result already absorbed by the plan with no ready successor is ACKed without a legacy wake TASK, preventing terminal or approval-gate loops.
When a plan task owner is a worker Host Bridge, include the collector/plan-dispatcher identity in that worker's `host_bridge_allowed_senders`; ACP does not spoof the coordinator identity. A missing allowlist entry fails before host invocation and leaves the TASK unacknowledged for safe retry.
Add the collector identity to the coordinator `host_bridge_allowed_senders`; wrapped results reply to the preserved original sender and do not loop back into the collector.
